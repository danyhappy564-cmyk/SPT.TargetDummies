...
- **Respawn Delay** — 죽고 나서 대체 마네킹이 나올 때까지의 시간. 그동안 시체가 그대로 누워있으니 시체 감상 시간이기도 합니다. 하한이 2초인 이유는, HollywoodFX가 래그돌이 자리 잡기를 기다렸다가 피를 재생하기 때문에 그보다 빨리 치우면 이펙트가 시작도 못 합니다.
- **Spawn Interval** — 6명을 한꺼번에 스폰할 때(사격장 입장, 리프레시) 하나와 다음 하나 사이의 간격. 죽은 뒤 리스폰과는 무관합니다(그건 Respawn Delay). 낮출수록 빠르지만 한 프레임에 작업이 몰립니다.
